axios.defaults.baseURL = "https://backend-redis.herokuapp.com/api";
...
if (document.querySelector("#scraps")) {
  ...
  (async function getScraps() {
    ...
    scraps.map((scrap, i) => {
      tBody.innerHTML += `
        <tr id="id_${scrap.uid}" class=${i % 2 === 0 ? "stripped" : ""}>
...
function createElement(scrap) {
  ...
  trDOM.id = `id_${scrap.uid}`;
  trDOM.innerHTML = `
  <td scope="row">${localStorage.getItem("username")}</td>
  <td>${scrap.title}</td>
  <td>${scrap.description}</td>
  <td>
  <a onclick="editTodo('${scrap.uid}')">Editar</a>
  <a onclick="deleteTodo('${scrap.uid}')">Excluir</a>
  </td>
  `;

  tBody.firstElementChild?.classList.contains("stripped")
    ? ""
    : (trDOM.classList = "stripped");
  return trDOM;
}
